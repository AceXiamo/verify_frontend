...
import type { FormDefine, FormItem } from '~/core/DataForm'
...
<template>
  <div :class="[$props.class]">
    <el-form ref="formRef" :inline="true" :label-width="labelWidth" :model="$props.modelValue.value" :rules="rules">
          ...
          <template v-for="(col, j) in row" :key="j">
            <el-col v-if="col.show ? col.show($props.modelValue.value) : true" :span="col?.col">
              <el-form-item class="w-full" :label="col?.label" :prop="(col as any).key">
                ...
                <template v-else>
                  <component :is="col?.type" class="!w-full" :readonly="$props.readonly || col.readonly" />
                </template>
                <template v-if="col?.remark">
                  <span class="whitespace-nowrap text-[12px] text-red-500">{{ col.remark }}</span>
                </template>
              </el-form-item>
            </el-col>
          </template>
